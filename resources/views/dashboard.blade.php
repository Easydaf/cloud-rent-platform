<x-app-layout>
    <div class="min-h-screen bg-cloud-bg text-cloud-dark py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-200 pb-6">
                <div>
                    ...
                    <p class="text-cloud-slate mt-1 text-sm font-medium">Pantau resource dan konfigurasi penyimpanan cloud terisolasi Anda secara real-time.</p>
                </div>
                <div>
                    <a href="{{ route('storage.packages') }}" class="inline-flex items-center px-5 py-2.5 bg-cloud-orange hover:bg-orange-600 text-white font-bold rounded-lg transition duration-200 transform hover:scale-105 shadow-md text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-cloud-card border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        @php
                        $used = (float) $activeSubscription->storage_used_gb;
                        $limit = (int) $activeSubscription->storage_limit_gb;
                        $percent = $limit > 0 ? min(100, max(0, ($used / $limit) * 100)) : 0;
                        $sisaKuota = max(0, $limit - $used);
                        @endphp
                        <div class="mb-4">
                            <span class="text-4xl font-black text-cloud-dark">{{ number_format($used, 1) }}</span>
                            <span class="text-cloud-slate text-lg font-bold"> / {{ $limit }} GB used</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                            <div class="bg-cloud-orange h-2.5 rounded-full" style="width: {{ $percent . '%' }};"></div>
                        </div>
                        <div class="text-xs text-cloud-orange font-semibold flex items-center gap-1 mt-4">
                            Tersisa sekitar {{ number_format($sisaKuota, 1) }} GB ruang kosong
                        </div>
                        @else
                        <div class="mb-4 py-2">
                            <p class="text-gray-400 text-sm font-medium">Belum ada kapasitas storage terdaftar.</p>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                ...
                <div class="bg-cloud-card border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        @if($activeSubscription)
                        <div class="mb-2">
                            <span class="text-lg font-extrabold text-cloud-dark bg-cloud-bg px-3 py-1 rounded-md border border-gray-200">
                                {{ $activeSubscription->package->package_name ?? 'Active' }} Tier
                            </span>
                        </div>
                        <p class="text-cloud-slate text-xs mt-3 leading-relaxed font-medium">
                            Masa aktif hingga: <span class="text-cloud-coral font-bold">{{ \Carbon\Carbon::parse($activeSubscription->expires_at)->translatedFormat('d F Y') }}</span><br>
                        ...
                        <div class="text-xs text-gray-400 mt-4 font-semibold">
                            ID Sewa: #SR-2026-{{ $activeSubscription->id }}
                        </div>
                        @else
                        <div class="mb-2">
                            <span class="text-sm font-bold text-gray-400 bg-gray-100 px-3 py-1 rounded-md border border-gray-200">Tidak Ada Paket</span>
                        </div>
                        <p class="text-gray-400 text-xs mt-3 leading-relaxed font-medium">Silakan lakukan penyewaan storage terlebih dahulu untuk mengaktifkan fitur kontainer otomatis.</p>
                        @endif
                    </div>
                </div>
                ...
                <div x-data="{ showKeys: false }" class="bg-cloud-card border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col justify-between lg:col-span-1 md:col-span-2">
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="bg-cloud-bg px-3 py-2 rounded-lg font-mono text-sm border border-gray-200 select-all tracking-wide text-cloud-dark">
                                @if(Auth::user()->aws_access_key_id)
                                <span x-show="showKeys">{{ Auth::user()->aws_access_key_id }}</span>
                                <span x-show="!showKeys" class="text-gray-400">••••••••••••••••••••</span>
                                @else
                                <span class="text-gray-300 italic">Belum digenerate</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="bg-cloud-bg px-3 py-2 rounded-lg font-mono text-sm border border-gray-200 select-all tracking-wide text-cloud-dark">
                                @if(Auth::user()->aws_secret_access_key)
                                <span x-show="showKeys" class="text-cloud-orange font-semibold">{{ Auth::user()->aws_secret_access_key }}</span>
                                <span x-show="!showKeys" class="text-gray-400">••••••••••••••••••••••••••••••••••••••••</span>
                                @else
                                <span class="text-gray-300 italic">Belum digenerate</span>
                                @endif
                    ...
                    <div class="text-[11px] text-cloud-slate font-medium flex items-center gap-1 mt-4">
                        ...
                        Bucket Name: <span class="font-bold text-cloud-orange font-mono">{{ Auth::user()->bucket_name ?? 'Belum ada' }}</span>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left text-sm text-cloud-slate">
                        ...
                        <tbody class="divide-y divide-gray-100 font-semibold">
                            ...
                            <tr class="hover:bg-cloud-bg hover:bg-opacity-50 transition">
                                <td class="p-4 text-cloud-dark">{{ $log->action }}</td>
                                <td class="p-4 font-mono text-xs text-cloud-orange">{{ $log->service }}</td>
                                <td class="p-4">
                                    <span class="px-2.5 py-0.5 rounded-full text-xs font-bold {{ $log->status === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                        {{ ucfirst($log->status) }}
                                    </span>
                                </td>
                                <td class="p-4 text-gray-400 font-medium">{{ $log->created_at->diffForHumans() }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="4" class="p-4 text-center text-gray-400 font-medium py-6">Belum ada riwayat aktivitas log terekam.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        @if (session('success'))
        <div class="mb-6 p-4 bg-green-100 border-l-4 border-green-500 text-green-700 rounded-r-lg font-semibold text-sm shadow-sm">
            {{ session('success') }}
        </div>
        @endif

        @if (session('error'))
        <div class="mb-6 p-4 bg-red-100 border-l-4 border-red-500 text-red-700 rounded-r-lg font-semibold text-sm shadow-sm font-mono">
            ⚠️ {{ session('error') }}
        </div>
        @endif
    </div>
